...
class Module extends Command {
	...
	listen() {
		...
		app.get('/api/player/:id/headshot', (request, response) => {
			...
			const headshotPath = path.resolve(__dirname, `../headshots/${id}.png`);
			if (!fs.existsSync(headshotPath)) {
				return response.status(404).json({ error: 'Player headshot is not available.' });
			}

			response.type('image/png');
			response.setHeader('Content-Disposition', `inline; filename="${id}.png"`);
			response.setHeader('Cache-Control', 'public, max-age=86400');
			return response.sendFile(headshotPath);
		});
