...
BEGIN
    ...
    - The underlying model matches the current odds pipeline:
      PLAYER_ODDS_FACTOR(...) for the base win probability
      and a fixed 5% margin when converting to decimal odds.
    ...
    */
    ...
    THEN
        ...
        WHERE 1 = 0;
    ELSE
        SET factorA = PLAYER_ODDS_FACTOR(resolvedPlayerA, resolvedPlayerB, normalizedSurface);
        ...
        ELSE
            ...
            ELSE
                SELECT
                    p.id AS player,
                    p.name AS name,
                    ROUND(1 / (factorA * 1.05), 2) AS odds
                FROM players p
                ...
                SELECT
                    p.id AS player,
                    p.name AS name,
                    ROUND(1 / (factorB * 1.05), 2) AS odds
                FROM players p
